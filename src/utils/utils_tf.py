...
def generate_true_false(
    ...
    """
    Generate true/false questions for specified book chapter using OpenSearch Serverless
    ...
    Returns:
        String containing the generated true/false questions
    """
    print(f"Generating {num_questions} true/false questions for chapter: {chapter_id}")
...
- Bloom's Level Guidelines: {guidelines}
    ...
    for i in range(min(num_questions, len(question_sequence))):
        ...
        answer = "TRUE" if i % 2 == 0 else "FALSE"  # Alternate between TRUE and FALSE
        question_obj = {
            "question_id": str(uuid.uuid4()),
            "statement": f"Sample true/false statement {i+1} about the chapter content (difficulty: {difficulty}, blooms: {blooms_level})",
            "answer": answer,
            "explanation": f"This statement is {answer.lower()}. This question tests {blooms_level} level thinking at {difficulty} difficulty based on the chapter content.",
            "difficulty": difficulty,
...
if __name__ == "__main__":
    ...
    print("Testing True/False generation with OpenSearch...")
